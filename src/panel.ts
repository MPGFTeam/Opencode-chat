  ...
  private async onMessage(msg: ViewMessage) {
    ...
    switch (msg.type) {
      ...
      case 'save-settings': {
        ...
        try {
          await vscode.workspace.getConfiguration('opencodeChat').update(key, value, vscode.ConfigurationTarget.Global);
          if (key === 'serverUrl') {
            this.rebuildClient();
            await this.subscribeEvents();
            await this.checkStatus();
      ...
      case 'get-models': {
        ...
        try {
          ...
          this.post('error', { message: `读取模型列表失败: ${this.errText(err)}` });
        }
        break;
      }
      case 'toggle-model': {
        ...
        try {
          await vscode.workspace.getConfiguration('opencodeChat').update('disabledModels', next, vscode.ConfigurationTarget.Global);
      ...
      case 'open-vscode-settings':
        vscode.commands.executeCommand('workbench.action.openSettings', 'opencodeChat');
        break;
      case 'export-session': {
        if (!this.currentSessionId) break;
        try {
          const messages = await client.messages(this.currentSessionId);
          const lines: string[] = [`opencode chat 导出 — 会话 ${this.currentSessionId}`, ''];
          ...
          const uri = await vscode.window.showSaveDialog({
            defaultUri: vscode.Uri.joinPath(vscode.workspace.workspaceFolders?.[0]?.uri ?? vscode.Uri.file(process.cwd()), `opencode-chat-${this.currentSessionId.slice(0, 8)}.txt`),
