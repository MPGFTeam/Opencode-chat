...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('opencodeChat.chat', panel),
    ...
    vscode.commands.registerCommand('opencodeChat.startServer', () => {
      vscode.commands.executeCommand('opencodeChat.chat.focus');
      panel.startServerFromCommand();
    }),
    vscode.commands.registerCommand('opencodeChat.checkStatus', () => {
      vscode.commands.executeCommand('opencodeChat.chat.focus');
        ...
        if (!ok) {
          vscode.window.showWarningMessage(
            `opencode server 的工作目录是 ${p.directory}，与当前工作区 ${workspaceFolder.uri.fsPath} 不一致。` +
              `AI 的文件操作会落在 server 目录。请在项目目录启动 serve 或调整 opencodeChat.serverUrl。`,
          );
        }
      } catch (err) {
