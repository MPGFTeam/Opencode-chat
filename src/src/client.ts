...
export class OpenCodeClient {
  ...
  async providers(): Promise<{ providerID: string; models: string[] }[]> {
    try {
      ...
      for (const p of all.providers || []) {
        const m = p.models || {};
        const ids = Array.isArray(m) ? m.map((x: any) => x.id).filter(Boolean) : Object.keys(m);
        list.push({ providerID: p.id, models: ids });
      }
      return list;
    } catch {
      return [];
  ...
  /** 回退方案：从 /provider 读已连接 provider 的模型列表 */
  async connectedModels(): Promise<{ providerID: string; models: string[] }[]> {
    const r = await this.json<any>('/provider');
    const all = r?.all ?? [];
    const conn: string[] = Array.isArray(r?.connected) ? r.connected : [];
    const out: { providerID: string; models: string[] }[] = [];
    for (const p of all) {
      if (!conn.includes(p?.id)) continue;
      const m = p?.models ?? {};
      const ids = Array.isArray(m) ? m.map((x: any) => x.id).filter(Boolean) : Object.keys(m);
      out.push({ providerID: p.id, models: ids });
    }
    return out;
  }
